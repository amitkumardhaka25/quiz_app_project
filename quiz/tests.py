...
class TestProgress(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='boncho',
                                             email='amitkumardhaka25@gmail.com',
                                             password='top_secret')
...
class TestNonQuestionViews(TestCase):
    ...
    def test_progress_user(self):
        user = User.objects.create_user(username='boncho',
                                        email='amitkumardhaka25@gmail.com',
                                        password='top_secret')
        question1 = MCQuestion.objects.create(content='squawk',
                                              category=self.c1)

        self.client.login(username='boncho', password='top_secret')
...
class TestQuestionMarking(TestCase):
    ...
    def setUp(self):
        self.c1 = Category.objects.new_category(category='elderberries')
        self.student = User.objects.create_user(username='boncho1',
                                                email='amitkumardhaka27@gmail.com',
                                                password='top_secret')
        self.teacher = User.objects.create_user(username='boncho2',
                                                email='amitkumardhaka45@gmail.com',
    ...
    def test_paper_marking_list_view(self):
        ...
        self.client.login(username='boncho1', password='top_secret')
        response = self.client.get('/marking/')
        self.assertRedirects(response, '/accounts/login/?next=/marking/',
                             status_code=302, target_status_code=404 or 200)

        self.client.login(username='boncho2', password='use_d@_force')
        response = self.client.get('/marking/')
        self.assertContains(response, 'test quiz 1')
        self.assertContains(response, 'test quiz 2')
        self.assertContains(response, 'boncho1')

    def test_paper_marking_list_view_filter_user(self):
        new_student = User.objects.create_user(username='boncho3',
                                               email='amitkumardhaka@gmail.com',
                                               password='maaaawwwww')
        boncho3_sitting = Sitting.objects.new_sitting(new_student, self.quiz1)
        boncho3_sitting.complete = True
        boncho3_sitting.save()

        self.client.login(username='boncho2', password='use_d@_force')
        response = self.client.get('/marking/',
                                   {'user_filter': 'Hans'})

        self.assertNotContains(response, 'boncho3')
        self.assertNotContains(response, 'boncho1')

        response = self.client.get('/marking/',
                                   {'user_filter': 'boncho3'})

        self.assertContains(response, 'boncho3')
        self.assertNotContains(response, 'boncho1')
...
class TestQuestionViewsUser(TestCase):
    ...
    def setUp(self):
        ...
        self.quiz_boncho4 = User.objects.create_user(username='boncho4',
                                                    email='amitkumardhaka26@gmail.com',
                                                    password='secret_top')
    ...
    def test_quiz_take_user_view_only(self):
        ...
        self.assertEqual(sitting.user.username, 'boncho')
    ...
    def test_normal_user_cannot_view_draft_quiz(self):
        ...
        self.client.login(username='boncho4', password='secret_top')
        ...
        self.quiz_boncho4.user_permissions.add(
